...
export default function Hero({ onOpenAuth, onOpenDashboard }: HeroProps) {
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);

  return (
    <section id="hero" className="relative flex flex-col items-center w-full bg-[#0A0A0A] py-16 px-6 md:py-[100px] md:px-[120px] overflow-hidden">
      {/* Badge */}
      <div className="flex items-center justify-center gap-[8px] h-[32px] px-[12px] md:px-[16px] bg-[#1A1A1A] border-2 border-[#FFD600]">
        ...
        <span className="font-ibm-mono text-[9px] md:text-[11px] font-bold text-[#FFD600] tracking-[1px] md:tracking-[2px] whitespace-nowrap">
          [NEW] // SMART INDIA HACKATHON 2026
        </span>
      </div>
      ...
      {/* Headline */}
      <h1 className="font-grotesk text-[clamp(32px,8vw,88px)] font-bold text-[#F5F5F0] tracking-[-1px] leading-none text-center w-full max-w-[1100px]">
        <GlitchText text="BRIDGING SKILLS." speed={45} delay={100} />
      </h1>
      <h1 className="font-grotesk text-[clamp(32px,8vw,88px)] font-bold text-[#FFD600] tracking-[-1px] leading-none text-center w-full max-w-[1100px]">
        <GlitchText text="BUILDING FUTURES." speed={45} delay={400} />
      </h1>

      <div className="h-8 md:h-[32px]" />

      {/* Subheading */}
      <p className="font-ibm-mono text-[13px] md:text-[15px] text-[#A0A09A] tracking-[1px] leading-[1.6] text-center w-full max-w-[850px]">
        REAL-TIME LABOUR-MARKET INTELLIGENCE THAT ALIGNS SKILL TRAINING WITH ACTUAL INDUSTRY DEMAND. FROM JOB SIGNAL TO CLASSROOM CURRICULUM.
      </p>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-[16px] w-full sm:w-auto">
        <button
          ...
          className="flex items-center justify-center w-full sm:w-[250px] h-[56px] bg-[#FFD600] hover:bg-[#e6c200] transition-colors border-none cursor-pointer"
        >
        ...
        <button
          ...
          className="flex items-center justify-center w-full sm:w-[230px] h-[56px] bg-[#0A0A0A] border-2 border-[#3D3D3D] hover:border-[#888888] transition-colors cursor-pointer"
        >
      ...
      <p className="font-ibm-mono text-[10px] md:text-[11px] text-[#666666] tracking-[1.5px] md:tracking-[2px] text-center">
        GOVT OF MAHARASHTRA INITIATIVE // 50+ SKILLS TRACKED // LIVE JOB DATA
      </p>
...
const layers = [
  { label: "SKILLSYNC // CORE", color: "#FFD600", indent: 0, active: true },
  { label: "JOB_SIGNALS", color: "#4ADE80", indent: 12 },
  { label: "NLP_EXTRACTOR", color: "#60A5FA", indent: 24 },
  { label: "CURRICULUM_GRAPH", color: "#FF6B35", indent: 24 },
  { label: "GAP_DETECTOR", color: "#FFD600", indent: 12 },
  { label: "DISTRICT_MAP", color: "#4ADE80", indent: 24 },
  { label: "POLICY_RECOMMENDER", color: "#888", indent: 24 },
  { label: "INSTITUTE_PORTAL", color: "#60A5FA", indent: 12 },
  { label: "EMPLOYER_LOOP", color: "#888", indent: 0 },
];

const inspectProps = [
  { key: "REGION", val: "PUNE / MH" },
  { key: "SKILLS", val: "50+ TRACKED" },
  { key: "POSTINGS", val: "50,000+" },
  { key: "ACCURACY", val: "94.2%" },
  { key: "ENGINE", val: "NLP + AI", swatch: "#FFD600" },
  { key: "STATUS", val: "ACTIVE", swatch: "#4ADE80" },
  { key: "REFRESH", val: "24 HOURS" },
  { key: "GOVT", val: "SIH26134" },
];

const tokens = [
  { name: "primary", hex: "#FFD600" },
  { name: "signal", hex: "#4ADE80" },
  { name: "gap", hex: "#FF6B35" },
...
const tickerItems = [
  "PYTHON", "DATA ANALYTICS", "MACHINE LEARNING", "FULLSTACK JS", "CNC MACHINING",
  "CYBERSECURITY", "CLOUD INFRA", "EMBEDDED C++", "PLC AUTOMATION", "ROBOTICS",
];
