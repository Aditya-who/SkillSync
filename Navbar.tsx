...
const links = [
  ...
  { label: "IMPACT",       section: "stats"       },
  { label: "CAPABILITIES", section: "capabilities"},
  { label: "WHY SKILLSYNC",section: "comparison"  },
    ...
    >
      <div className="flex items-center justify-between h-[60px] px-6 md:px-[48px] max-w-[1400px] mx-auto">
        ...
        <a href="#" className="flex items-center gap-[10px] shrink-0 group">
          ...
          <span className="font-grotesk text-[14px] font-bold text-[#F5F5F0] tracking-[2.5px]">
            SKILLSYNC
          </span>
        </a>
          ...
          {links.map(({ label, section }) => {
            ...
            return (
              <button
                ...
                style={{ color: isActive ? "#FFD600" : "#777" }}
                onMouseEnter={(e) => {
                  if (!isActive) (e.currentTarget as HTMLButtonElement).style.color = "#F5F5F0";
                }}
                onMouseLeave={(e) => {
                  (e.currentTarget as HTMLButtonElement).style.color = isActive ? "#FFD600" : "#777";
                }}
              >
                {label}
                <span
        ...
        <div className="hidden lg:flex items-center gap-[14px]">
          <button
            onClick={() => onOpenAuth ? onOpenAuth() : scrollTo("faq")}
            className="font-ibm-mono text-[10px] text-[#777] tracking-[1.5px] hover:text-[#F5F5F0] transition-colors bg-transparent border-none cursor-pointer"
          >
            LOG IN
          </button>
          <button
            onClick={() => onOpenAuth ? onOpenAuth() : scrollTo("hero")}
            className="font-grotesk text-[11px] font-bold text-[#0A0A0A] bg-[#FFD600] tracking-[1.5px] px-[18px] py-[9px] hover:bg-[#F5F5F0] transition-colors border-none cursor-pointer"
          >
            GET STARTED
          </button>
        </div>
      ...
      >
        <nav className="flex flex-col px-6 py-5 gap-0">
          {links.map(({ label, section }) => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-2 w-full font-ibm-mono text-[12px] tracking-[2px] py-[12px] border-b border-[#141414] transition-colors bg-transparent border-x-0 border-t-0 cursor-pointer"
          ...
          <div className="flex flex-col gap-[10px] pt-4">
            <button
              onClick={() => { setMenuOpen(false); if (onOpenAuth) onOpenAuth(); }}
              className="font-ibm-mono text-[12px] text-[#555] tracking-[1.5px] text-left bg-transparent border-none cursor-pointer"
            >
              LOG IN
            </button>
            <button
              onClick={() => { setMenuOpen(false); if (onOpenAuth) onOpenAuth(); }}
              className="font-grotesk text-[11px] font-bold text-[#0A0A0A] bg-[#FFD600] tracking-[1.5px] px-[18px] py-[11px] text-center hover:bg-[#F5F5F0] transition-colors border-none cursor-pointer"
            >
              GET STARTED
            </button>
          </div>
